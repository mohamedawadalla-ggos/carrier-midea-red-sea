...
export function LocationsPanel({ data, refresh }: { data: ControlPanelSnapshot; refresh: () => Promise<void> }) {
  ...
  async function addCity(event: FormEvent<HTMLFormElement>) {
    ...
    const publish = data.profile.role === "super_admin" || data.profile.role === "management";
    const cityName = String(form.get("nameEn"));
    if (publish && !window.confirm("Add and publish " + cityName + " as a service city immediately?")) return;
    const { error } = await getSupabase().from("service_locations").insert({ slug: String(form.get("slug")), name_ar: String(form.get("nameAr")), name_en: cityName, governorate_ar: String(form.get("govAr")), governorate_en: String(form.get("govEn")), display_order: data.locations.length + 1, status: publish ? "published" : "pending_approval", approved_by: publish ? data.profile.user_id : null });
    if (error) setMessage(error.message); else { event.currentTarget.reset(); setMessage(publish ? "City added and published." : "City added for approval."); await refresh(); }
  }
