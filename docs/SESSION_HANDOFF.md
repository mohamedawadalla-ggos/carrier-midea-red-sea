...
- **`Adel Mina`'s role**: per Nael's request, should be `super_admin` for this phase (was `management`). **Not yet done as of this writing** — needs the admin panel's Users tab (role change should go through the panel's own update flow, which has the confirmation dialog from PR #24 and audit logging, not a raw database write). Confirm it landed before closing this line.
