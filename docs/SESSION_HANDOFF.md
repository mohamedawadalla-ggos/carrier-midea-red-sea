...
- Toggle "Leaked Password Protection" on in Supabase Auth settings (one click, flagged as disabled in an earlier audit, never reconfirmed). Confirmed 2026-08-05: not safely doable via CLI (not representable in `supabase/config.toml`; pushing config risks resetting other undeclared dashboard-only auth settings) — needs the one-click dashboard toggle from a human.
