...
- **`SUPABASE_ACCESS_TOKEN` and `SUPABASE_PROJECT_REF` secrets are now added** (2026-08-04) and the `Deploy Supabase` workflow (PR #27) was triggered for the first time — `supabase link` succeeded (auth/project ref both good), but `supabase db push` **failed** with: `Remote migration versions not found in local migrations directory`, naming six unrecognized remote timestamps: `20260726000000 20260726071050 20260726071131 20260726144235 20260726192909 20260726193441`. **Nothing was changed on the live database** — the CLI aborted before applying anything, and the Edge Functions deploy step was correctly skipped as a result. This means the live database has migration history for changes that don't exist as files in this repo's `supabase/migrations/` — almost certainly ad-hoc changes applied directly (SQL editor or local CLI) during the pre-handoff "Codex" era, never committed to git. **Do not run `supabase migration repair --status reverted ...` blindly** — that marks those 6 as "never happened," which is only correct if verified true. Next step needs someone with real dashboard/CLI access to run `supabase db pull` (read-only) and diff what it generates against this repo, to figure out what those 6 migrations actually are before deciding whether to import them into the repo or repair/discard the tracking records. Only after that's resolved can `supabase db push` succeed and the real backlog (§26/#25's migrations, plus whatever else is actually missing) get applied.
