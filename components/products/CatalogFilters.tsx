...
import { catalogSortValues, emptyCatalogFilters, formatHorsepower, supportedHorsepowerValues, type CatalogFilterState, type CatalogSortValue } from "@/lib/catalog-filtering";

export { emptyCatalogFilters, type CatalogFilterState } from "@/lib/catalog-filtering";

export function CatalogFilters({ locale, value, sort, onChange, onSortChange, lockProductType }: { locale: Locale; value: CatalogFilterState; sort: CatalogSortValue; onChange: (next: CatalogFilterState) => void; onSortChange: (next: CatalogSortValue) => void; lockProductType?: boolean }) {
  ...
  return <div className="product-filters catalog-filters" aria-label={ar ? "تصفية عائلات المنتجات" : "Product-family filters"}>
    ...
    <label>{ar ? "الترتيب" : "Sort by"}<select value={sort} onChange={(event) => onSortChange(event.target.value as CatalogSortValue)}>{catalogSortValues.map((value) => <option key={value} value={value}>{({ "hp-asc": ar ? "القدرة: الأقل أولاً" : "HP: low to high", "hp-desc": ar ? "القدرة: الأعلى أولاً" : "HP: high to low", "product-type": ar ? "نوع الجهاز" : "Product type", brand: ar ? "العلامة" : "Brand", technology: ar ? "التقنية" : "Technology" } as const)[value]}</option>)}</select></label>
