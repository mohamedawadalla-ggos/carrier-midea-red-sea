...
export function ProductFamilyCard({ family, variants, locale }: { family: ProductFamily; variants: ProductVariant[]; locale: Locale }) {
  return <article className="product-card family-card">
    ...
    <div className="product-card-body"><p className="product-series">{family.technology === "inverter" ? "Inverter" : pick(locale, "ثابت السرعة", "Fixed speed")} · {family.refrigerant}</p><h3><Link href={`/${locale}/products/${family.productType}/${family.slug}`}>{family.name[locale]}</Link></h3><p>{family.description[locale]}</p><div className="product-specs"><span>{variants.length} {pick(locale, "موديلات", "models")}</span><span>{pick(locale, "اطلب السعر الحالي", "Request Current Price")}</span></div><Link className="product-price family-link" href={`/${locale}/products/${family.productType}/${family.slug}`}>{pick(locale, "عرض العائلة والموديلات", "View family and models")}</Link></div>
  </article>;
}
