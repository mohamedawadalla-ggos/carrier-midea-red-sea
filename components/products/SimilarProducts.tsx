...
export function SimilarProducts({ family, variants, locale }: { family: ProductFamily; variants: ProductVariant[]; locale: Locale }) {
  ...
  if (liveCatalog.status !== "disabled" && liveCatalog.status !== "ready") return null;
  const active = variants.filter((variant) => variant.active && liveCatalog.variants.some((item) => item.modelCode === variant.modelCode)).sort((left, right) => left.capacityHp - right.capacityHp || left.displayOrder - right.displayOrder);
  const reference = active[0];
  if (!reference) return null;
  const suggestions = rankRelatedProducts(liveCatalog.families, liveCatalog.variants, { familyId: family.id, variantId: reference.id, horsepower: reference.capacityHp }).slice(0, 4);
