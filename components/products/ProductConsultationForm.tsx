...
export function ProductConsultationForm({ family, variants, locale, selectedModel }: { family: ProductFamily; variants: ProductVariant[]; locale: Locale; selectedModel?: string }) {
  ...
  return <form className="lead-form product-inquiry" id="inquiry" onSubmit={submit}>
    ...
    <label>{ar ? "الموديل" : "Selected model"}<select name="model" defaultValue={selectedModel ?? variants[0]?.id}>{variants.map((variant) => <option key={variant.id} value={variant.id}>{formatHorsepower(locale, variant.capacityHp)} · {variant.coolingMode === "cool-only" ? (ar ? "بارد فقط" : "Cool only") : (ar ? "بارد / ساخن" : "Cool & heat")}</option>)}</select></label><label>{ar ? "هل تحتاج إلى التركيب؟" : "Installation required?"}<select name="installation"><option value="yes">{ar ? "نعم" : "Yes"}</option><option value="no">{ar ? "لا" : "No"}</option></select></label><label className="full">{ar ? "ملاحظات إضافية" : "Additional notes"}<textarea name="notes" rows={3} /></label>
