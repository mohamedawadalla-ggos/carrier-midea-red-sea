...
export function AdvisorDialog({ locale, open, titleId, children, onClose }: { locale: Locale; open: boolean; titleId: string; children: ReactNode; onClose: () => void }) {
  ...
  return <dialog ref={dialogRef} className="coolpet-dialog" aria-labelledby={titleId} onCancel={onClose} onClose={() => { if (open) onClose(); }} onClick={(event) => { if (event.target === dialogRef.current) onClose(); }}>
